...
**갱신(2026-09-01) — 위 완화·설계는 이제 완료됐다, 단 노출은 여전히 안 함**: 사용자가 순차 승인해 (1) 완화방안 설계, (2) 그 설계를 실제 코드로 구현+픽스처 실측까지 마쳤다 — `pipeline-codegen.mjs`의 `registerAndVerifyComponent()`(등록 직후 자동 재검증 연쇄, FAIL 시 `isError:true` 응답 설계 포함), 상세 근거·테스트 결과는 `T2B_RISK_REVIEW.md` §5-1·§6 참조(여기 중복 기술 안 함). **`scripts/mcp-server.mjs`는 이 갱신 시점에도 여전히 무수정** — `register_component_preview`에 T2b 경로를 추가하지 말라는 위 문장의 결론 자체는 그대로 유효하다. 남은 선행조건은 "사용자의 명시적 승인" 하나뿐이다.
...
**간접 의존성까지 포함한 전체 126개 패키지 라이선스 전수 감사를 완료했다(2026-08-21, "다음 Phase 작업" 재검토 중 착수)**: `03_PHASES.md` Phase 3 "베타 공개 준비" 체크리스트에 있던 "ScanCode 1회 스캔" 항목을 준비하다가, `ScanCode Toolkit`이 이 환경의 Python 3.14를 지원하지 않는다는 걸 확인했다(공식적으로 Python 3.12까지만 지원 — 별도 구버전 설치가 필요해져 배보다 배꼽인 상황). 같은 목적(라이선스 컴플라이언스 확인)을 Node.js로 직접 구현해 대체했다: `node_modules`의 모든 패키지(직접 의존성 8개가 데려오는 간접 의존성까지 합쳐 실제로는 **126개**)의 `package.json`의 `license` 필드를 전수 대조했다. 지금까지 README·`02_DATA_MODEL.md`가 실측 확인해온 건 직접 의존성 8개뿐이었고, 118개 간접 의존성은 한 번도 검증된 적이 없었다.

**결과**: license 필드가 없는 패키지 0개, GPL·AGPL 등 강한 카피레프트 라이선스 0개(MIT 103·ISC 10·Apache-2.0 4·MPL-2.0 3·BSD-3-Clause 2·BSD-2-Clause 1·0BSD 1로 전부 permissive). **발견 1건**: `sharp`가 함께 설치하는 플랫폼별 네이티브 바이너리 패키지 2개(`@img/sharp-win32-x64`·`@img/sharp-wasm32`)가 `Apache-2.0 AND LGPL-3.0-or-later`를 포함한다 — 내부적으로 `libvips`를 동적 링크하기 때문이다. **법적 판단**: 문제없음. LGPL은 코드를 직접 복사·수정하지 않고 동적 링크만 하는 경우 그 라이선스가 링크하는 프로그램(이 킷) 전체로 전이되지 않는 것을 명시적으로 허용하는 조항을 두고 있고, 이 킷은 `sharp`를 수정 없이 설치 의존성으로만 쓰므로 이 조건에 그대로 해당한다.
